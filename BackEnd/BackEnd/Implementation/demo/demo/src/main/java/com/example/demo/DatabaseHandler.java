...
public class DatabaseHandler {
    ...
    /* The checkStaffLogin method takes in a staff username and password and checks 
       if the username and password match an existing staff record in the database. 
       It returns a boolean value indicating whether the login was successful or not. 
    */
    public boolean checkStaffLogin(String staffUsername, String staffPassword) {
        
        try (
            Connection connection = DriverManager.getConnection(getDbUrl(), getUser(), getPassword()); 
            Statement statement = connection.createStatement();
            ResultSet results = statement.executeQuery("SELECT sUsername, sPassword FROM Staff"); 
        ) { while (results.next()) {
                if (staffUsername.equals(results.getString("sUsername")) && staffPassword.equals(results.getString("sPassword"))) {
            ...
            System.out.println("Connection Failed. Login data not retrieved/validated.");
    ...
    //with wallet ID get call create wallet. Make customer with these parameters
        ...
        try (
            ...
            ResultSet results = statement.executeQuery("SELECT * FROM Customer WHERE custID=" + customerLocalID); 
                ...
                //Customer customer = new Customer(pin,passw,new Wallet("seed"));
                //System.out.println("what is 2: "+customer.getPin()+""+customer.getPassword());
                return customer;
            }
            statement.close();
            connection.close();
       ...
       public Wallet createWallet(int walletID)
    {
        try (
            ...
            ResultSet cryptoResults = statement3.executeQuery("SELECT * FROM Cryptocurrency WHERE walletID =" + walletID);
                ...
                //iterate through cryptocurrency table and do what's neeeded
                ...
                //iterate through transaction and do what's needed
        ...
        try (
            ...
            for (int i=startingHour; i<=endingHour-1; i++) {
                ...
                if (add) { 
                    ...
                    c.setTimeInMillis(allBookedSessionsCS.get(0).getSessionDateTime().getTimeInMillis());
                    c.set(Calendar.HOUR_OF_DAY, i);
                    c.set(Calendar.MINUTE, 0);
                    c.set(Calendar.SECOND, 0);
                    availableSessionsToday.add(c);
                }
            }            
            statement.close();
            connection.close();
    ...
    */
    public HashMap<String,String> getCustomerKeys()
    {
        ...
        try (
            ...
            ResultSet keys = statement.executeQuery("SELECT Crypto.cryptoName, publicAddress, privateKey "
                ...
                //keysString.add(keys.getString("cryptoName")+" "+keys.getString("publicAddress")+" "+keys.getString("privateKey"));
            }
            statement.close();
            connection.close();
        }catch (SQLException except) {
    ...
    public int getTransactionId(Transaction transaction)
    {
        ...
        try (
            ...
            int trasnactionID = transactionIDResult.getInt("tID");
            statement.close();
            connection.close();
